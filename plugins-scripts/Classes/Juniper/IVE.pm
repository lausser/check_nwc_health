...
sub analyze_mem_subsystem {
  ...
  $self->{components}->{mem_subsystem} =
      Classes::Juniper::IVE::Component::MemSubsystem->new();
}

sub analyze_cpu_subsystem {
  my $self = shift;
  $self->{components}->{cpu_subsystem} =
      Classes::Juniper::IVE::Component::CpuSubsystem->new();
}

sub analyze_environmental_subsystem {
  my $self = shift;
  $self->{components}->{environmental_subsystem} =
      Classes::Juniper::IVE::Component::EnvironmentalSubsystem->new();
}

sub analyze_user_subsystem {
  my $self = shift;
  $self->{components}->{user_subsystem} =
      Classes::Juniper::IVE::Component::UserSubsystem->new();
}
