...
sub init {
  ...
  if ($self->mode =~ /device::hardware::health/) {
    ...
    $self->analyze_and_check_cm_subsystem("Classes::CiscoCCM::Component::CmSubsystem");
  } elsif ($self->mode =~ /device::phone/) {
    $self->analyze_and_check_phone_subsystem("Classes::CiscoCCM::Component::PhoneSubsystem");
  } else {
    $self->no_such_mode();
  }
}
